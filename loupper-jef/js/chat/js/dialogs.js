...
function showOrUpdateDialogInUI(itemRes, updateHtml) {
  ...
  var dailogLastMessageSentDate = itemRes.last_message_date_sent;
//console.log("Dialog:-"+JSON.stringify(itemRes));
  if (dialogType == 3) {
  ...
  if (updateHtml === true) {
  	var updatedDialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage,dailogLastMessageSentDate);
  	$('#dialogs-list').prepend(updatedDialogHtml);
  	$('.list-group-item.active .badge').text(0).hide(0);

	} else {
    var dialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage,dailogLastMessageSentDate);
...
function getDialogIcon (dialogType) {
  var groupPhoto = '<img src="../js/chat/images/ava-group.svg" width="50" height="50" class="round">';
  var privatPhoto  = '<img src="../js/chat/images/ava-single.svg" width="50" height="50" class="round">';
